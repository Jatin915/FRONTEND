...
const scoresArray = ["85", 92, "67", 40, 100, "30", 76, "90"]
...
function analyzeScores(scoresArray) {
  ...
  for (let i = 0; i < scoresArray.length; i++) {
    const num = Number(scoresArray[i]);
    ...
    if (!isNaN(num)) {
      ...
      if (num >= 90) {
        ...
        num >= 60 ? passCount++ : failCount++; 
      }
    }
  }

  const average = validCount === 0 ? 0 : Number((totalScore / validCount)).toFixed(2);
    return {
...
const result = analyzeScores(scoresArray);
console.log(result);